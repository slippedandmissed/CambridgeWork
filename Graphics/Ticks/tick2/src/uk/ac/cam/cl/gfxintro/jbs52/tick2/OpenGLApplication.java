...
import uk.ac.cam.cl.gfxintro.jbs52.entities.scenes.CompetitionScene;
...
public class OpenGLApplication {
	...
	public void initialize() throws Exception {
		...
		scene = new CompetitionScene(skybox, camera);
		scene.populate();
	...
	private void initializeInputs() {
		...
		// screenshot, "V" - capture frames for video, "Space" - Pause animation
		key_cb = new GLFWKeyCallback() {
			public void invoke(long window, int key, int scancode, int action, int mods) {
				if (key == GLFW_KEY_W && action == GLFW_PRESS) {
					...
					System.out.println(camera.getElevation());
	...
	public void render() {
		...
		float deltaTime = (newTime - currentTime) / 1000.f; // Time taken to render this frame in seconds (= 0 when the
		...
		if (!pause) {
			...
			scene.render(camera, deltaTime, elapsedTime);

		} else {
			skybox.render(camera, 0f, elapsedTime);
			scene.render(camera, 0f, elapsedTime);
		}
